<script lang="ts">
  ...
  import { Save, Download, Upload, AlertTriangle, Users, BookOpen, GraduationCap, Plus, Trash2, ArrowLeft, CheckCircle2, XCircle, GripVertical } from "@lucide/svelte";
  ...
  $effect(() => {
    horario; configDocentes; docentesLocales; materiasLocales; gruposLocales;
    guardarStorage();
  });
  ...
  function exportarJSON() {
    const filas: HorarioDocentePlano[] = Object.values(horarioDocentePlano).filter((f) =>
      DIAS.some((d) => f[d].some((s) => s !== ""))
    );
