<script lang="ts">
  ...
  export let colorTheme: "blue" | "green" | "purple" = "blue"; // Default to blue
...
{#if showPopup}
  ...
  >
          ...
          <div class="relative">
            <div
              class="absolute w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent {popupClasses.arrowBorder[colorTheme]}"
              style="top: -8px; left: 50%; transform: translateX(-50%);"
            ></div>
    
            <!-- Contenido del popup -->
            <div
              class="bg-gradient-to-r {popupClasses.bgGradientFrom[colorTheme]} {popupClasses.bgGradientTo[colorTheme]} text-white p-4 rounded-xl shadow-2xl border {popupClasses.borderColor[colorTheme]} min-w-[280px] max-w-[calc(100vw-20px)] md:max-w-[320px]"
            >
              ...
              <div class="flex gap-2">
                <button
                  on:click={onTryNow}
                  class="px-3 py-2 rounded-lg text-sm bg-white {popupClasses.buttonTextColor[colorTheme]} font-semibold {popupClasses.buttonHoverBg[colorTheme]} transition-colors"
                >
                  Probar ahora
                </button>
                <button
