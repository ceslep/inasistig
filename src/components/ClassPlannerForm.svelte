...
Los tiempos son en minutos y deben sumar entre 60 y 80. Tema: ${aiPrompt}`
    ...
    async function handleSubmit() {
        ...
        if (guardadoOnline) {
            showLastSaved({
              fecha: planeacionData.fecha_creacion || '',
              docente: planeacionData.docente,
              materia: planeacionData.subject || '',
              grado: planeacionData.grade || '',
              periodo: planeacionData.period || '',
...
<ModuleHeader title="Planeador de Clases" subtitle="Estándares · DBA · Colombia" {onBack}>
    {#snippet actions()}
        <button
            onclick={() => showFirmaModal = true}
            class="min-h-[44px] px-3 py-2 rounded-xl transition-all duration-200 flex items-center gap-2 text-sm font-medium
        ...
        <button
            ...
            class="min-h-[44px] px-3 py-2 rounded-xl bg-sky-500/10 text-sky-600 border border-sky-500/30 transition-all duration-200 flex items-center gap-2 text-sm font-medium disabled:opacity-60"
...
{#if lastSaved && lastSavedVisible}
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs font-semibold" style="color: rgb(var(--text-primary));">Última planeación guardada</p>
        <p class="text-[11px] mt-1 opacity-70" style="color: rgb(var(--text-secondary));">
          {lastSaved.fecha} &bull; {lastSaved.grado} &bull; {lastSaved.periodo}
        </p>
        <p class="text-[11px] truncate opacity-60" style="color: rgb(var(--text-secondary));" title={lastSaved.materia}>
          {lastSaved.materia}
        </p>
